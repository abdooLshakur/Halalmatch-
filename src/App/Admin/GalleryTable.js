...
const GalleryTable = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100">
      <ToastContainer />
      <Sidebar />
      <div className="w-full md:w-[85vw] p-6">
        <div className="w-full px-4 py-6 bg-white bg-opacity-90 backdrop-blur-sm shadow-md rounded-lg m-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
            <h2 className="text-2xl font-bold text-gray-800">Gallery</h2>
            <input
              type="text"
              placeholder="Search by header or location"
              className="border border-gray-300 shadow-sm p-2 rounded w-full sm:w-64 focus:ring-2 focus:ring-purple-300 focus:outline-none bg-white"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <Link to={"/CreateGallery"}>
              <button className="text-white bg-green-600 hover:bg-green-700 px-4 py-2 rounded shadow">
                Create
              </button>
            </Link>
          </div>
